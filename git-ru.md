...
git diff <file>  # показывает различия между рабочим каталогом и индексом только для указанного файла
git diff <commit-hash-1> <commit-hash-2>  # показывает различия между двумя коммитами
git diff <commit-hash-1> <commit-hash-2> <file>  # показывает различия в конкретном файле между двумя коммитами
...
git diff --name-only  # показывает только имена изменённых файлов, без содержимого
git diff --stat  # показывает сводную статистику по измененным файлам (добавленные/удалённые строки)
