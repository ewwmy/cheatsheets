...
git commit --amend # добавить изменения в предыдущий коммит и отредактировать его сообщение в интерактивном режиме
git commit --amend -m "Комментарий для коммита" # добавить изменения в предыдущий коммит и отредактировать его сообщение (обычно используется для редактирования сообщения последнего коммита)
git commit --amend --no-edit # добавить изменения в предыдущий коммит без редактирования его сообщения
```
...
> Удаление всех незакоммиченных данных.

```bash
git reset --hard # удалить все незакоммиченные изменения и сбросить индекс (вернуться к исходному состоянию последнего коммита, на который указывает HEAD)
git reset --hard HEAD~1 # сбросить текущую ветку на один коммит назад и удалить все несохраненные изменения
git reset --hard <commit_hash> # вернуться к конкретному коммиту по его хэшу
git reset --hard ORIG_HEAD # вернуться к состоянию ветки до git rebase или git merge (ORIG_HEAD — это указатель на коммит перед последним изменением)
```
...
```bash
git rebase feature-1 # перенос ветки feature-1 в текущую ветку
```
...
> Позволяет создавать новые рабочие каталоги одного репозитория и работать с ними, не беспокоясь о необходимости делать `stash` или даже ненужные коммиты с промежуточными изменениями.
