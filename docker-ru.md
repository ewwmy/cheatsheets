...
- сервера с длительно работающим демоном-процессом **`dockerd`**, который создает образы, контейнеры, тома и управляет ими.
...
Пример Dockerfile для бекенд-приложения на Node.js с использованием кеширования:
...
Чтобы использовать возможности кеширования, сначала копируем и устанавливаем зависимости, так как они меняются значительно реже, чем код (или исполняемые файлы) самого приложения. В этом случае, если зависимости не изменились, при последующей сборке они будут скопированы из кеша.

В общем случае, для использования возможностей кеширования, сначала выполняем шаги, которые затрагивают **наименее** изменяемые данные.

Пример Dockerfile для бекенд-приложения на Node.js с использованием кеширования и `.dockerignore`:
...
- на следующем этапе в качестве базового образа выбираем образ ОС без лишних зависимостей, например, `alpine`
	- в качестве базового образа можно использовать образ из предыдущей стадии, например: `FROM builder`, а также присвоить ему алиас: `FROM builder AS final`, чтобы уже текущий образ на следующей стадии и т.д.
- для копирования файлов с предыдущих ступеней используется `COPY` с инструкцией `--from`, где задается алиас образа, откуда необходимо скопировать данные.
...
#docker #конспект #cheatsheet #my #lecture #notes #compose 
